...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName(`generatefirematerials`)
        .setDescription(`FireMaterial-Generierung anstoßen`)
    ),
        ...
        const activeCheck = await fetch(
          ...
          .then(async (res) => {
            if (res.data.running) {
              ...
              await interaction.deleteReply().catch((error) => {});
              const checkMessage = await channel.send(
                "Möchtest du die Generierung der FireMaterials wirklich anstoßen?"
              );
              checkMessage.react("✅").then((r) => {
                checkMessage.react("❌");
              });
              ...
              checkMessage
                ...
                .then(async (collected) => {
                  ...
                  if (reaction.emoji.name === "✅") {
                    ...
                    if (response.status < 300) {
                      msg = await channel.send(
                        "⚠️ Die Generierung der FireMaterials wurde erfolgreich angestoßen..."
                      );
                      start();
                    } else {
                      msg = await channel.send(
                        "❌ FEHLER | Die Generierung der FireMaterials konnte nicht gestartet werden!"
                      );
                      setTimeout(() => {
                        msg.delete().catch((error) => {});
                      }, 4000);
                        ...
                        .then((res) => {
                          if (res && !res.data.running) {
                            if (res.data.successful) {
                              ...
                              channel.send(
                                `✅ Die Generierung der FireMaterials wurde erfolgreich in ${duration} abgeschlossen!`
                              );
                            } else {
                              msg.delete();
                              channel.send(
                                "⚠️ FEHLER | Die Generierung der FireMaterials war nicht erfolgreich!"
                              );
                            }
                            clearInterval(intervalId);
                          }
                    ...
                    const msg = await channel.send(
                      "❌ FireMaterials Generierung abgebrochen"
                    );
                    checkMessage.delete().catch((error) => {});
                    setTimeout(() => {
                      msg.delete().catch((error) => {});
                ...
                .catch(async (collected) => {
                  const msg = await channel.send(
                    "❌ FireMaterials Generierung abgebrochen! Keine Reaktion innerhalb von 10 Sekunden ❌"
                  );
                  checkMessage.delete().catch((error) => {});
                  setTimeout(() => {
                    msg.delete().catch((error) => {});
